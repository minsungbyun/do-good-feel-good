...
public class OrgUserUpdateHandler implements Command {

  OrgDao orgDao;

  public OrgUserUpdateHandler(OrgDao orgDao) {
    this.orgDao = orgDao;
  }
  ...
  public void execute(CommandRequest request) throws Exception {
    ...
    OrgDTO loginUser = orgDao.findByIdPassword(AuthLoginHandler.getLoginUser().getId(), userPassword);
    ...
    String corpNo = Prompt.inputString("사업자등록번호(" + loginUser.getCorpNo() + ")? ");
    String fax = Prompt.inputString("팩스(" + loginUser.getFax() + ")? ");
    String hpmepage = Prompt.inputString("홈페이지(" + loginUser.getHomepage() + ")? ");
    ...
    loginUser.setCorpNo(corpNo);
    loginUser.setFax(fax);
    loginUser.setHomepage(hpmepage);
    loginUser.setPassword(password);


    orgDao.update(loginUser);
    //    orgDao.updateOrg(loginUser);
