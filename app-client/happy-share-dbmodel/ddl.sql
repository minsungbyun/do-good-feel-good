...
CREATE TABLE ftp_user_personal (
  ...
  rank             VARCHAR(50) NOT NULL DEFAULT 천콩이 COMMENT '등급', -- 등급
...
-- 답변
CREATE TABLE TABLE (
  COL    <데이터 타입 없음> NOT NULL COMMENT '답글번호', -- 답글번호
  qna_no INTEGER            NULL     COMMENT '문의하기게시판번호', -- 문의하기게시판번호
  COL2   <데이터 타입 없음> NULL     COMMENT '새 컬럼', -- 새 컬럼
  COL3   <데이터 타입 없음> NULL     COMMENT '새 컬럼2' -- 새 컬럼2
)
COMMENT '답변';

-- 답변
ALTER TABLE TABLE
  ADD CONSTRAINT PK_TABLE -- 답변 기본키
    PRIMARY KEY (
      COL -- 답글번호
    );
...
-- 답변
ALTER TABLE TABLE
  ADD CONSTRAINT FK_ftp_support_qna_TO_TABLE -- 문의하기 -> 답변
    FOREIGN KEY (
      qna_no -- 문의하기게시판번호
    )
    REFERENCES ftp_support_qna ( -- 문의하기
