...
CREATE TABLE ftp_user (
	...
	donation       INTEGER      NULL     COMMENT '기부금액', -- 기부금액
...
CREATE TABLE ftp_vol_apply (
	...
	applyl_dt    DATE    NOT NULL DEFAULT CURDATE() COMMENT '신청일', -- 신청일
	start_time   TIME    NOT NULL COMMENT '시작시간', -- 시작시간
	end_time     TIME    NOT NULL COMMENT '종료시간', -- 종료시간
	apply_status INTEGER NOT NULL DEFAULT 1 COMMENT '상태', -- 상태
...
CREATE TABLE ftp_challenge (
	...
	deadline_dt   DATE         NULL     COMMENT '총챌린지일수', -- 총챌린지일수
	deadline_time DATETIME     NULL     COMMENT '남은시간', -- 남은시간
	current_cnt   INTEGER      NULL     COMMENT '현재참여인원', -- 현재참여인원
	review_cnt    INTEGER      NULL     DEFAULT 0 COMMENT '참여댓글수', -- 참여댓글수
	qna_cnt       INTEGER      NULL     DEFAULT 0 COMMENT '문의갯수' -- 문의갯수
)
COMMENT '챌린지';
...
CREATE TABLE ftp_support_qna (
	...
	user_no         INTEGER      NOT NULL COMMENT '회원번호', -- 회원번호
	qna_category_no INTEGER      NULL     COMMENT '카테고리번호', -- 카테고리번호
	title           VARCHAR(255) NOT NULL COMMENT '제목', -- 제목
	content         TEXT         NOT NULL COMMENT '내용', -- 내용
	password        VARCHAR(100) NOT NULL COMMENT '문의비밀번호', -- 문의비밀번호
	create_dt       DATE         NOT NULL DEFAULT CURDATE() COMMENT '등록일', -- 등록일
	view_cnt        INTEGER      NULL     DEFAULT 0 COMMENT '조회수', -- 조회수
	qna_reply       TEXT         NULL     DEFAULT 0 COMMENT '답글' -- 답글
)
COMMENT '문의하기';
...
CREATE TABLE ftp_vol (
	...
	start_time      TIME         NOT NULL COMMENT '시작시간', -- 시작시간
	end_time        TIME         NOT NULL COMMENT '종료시간', -- 종료시간
	deadline_dt     DATE         NULL     COMMENT '총봉사일수', -- 총봉사일수
	deadline_time   DATETIME     NULL     COMMENT '남은시간', -- 남은시간
	current_cnt     INTEGER      NULL     COMMENT '현재참여인원', -- 현재참여인원
	...
	qna_cnt         INTEGER      NULL     DEFAULT 0 COMMENT '문의댓글갯수' -- 문의댓글갯수
)
COMMENT '봉사';
...
CREATE TABLE ftp_funding (
	...
	start_dt            DATE         NOT NULL COMMENT '시작일', -- 시작일
	end_dt              DATE         NOT NULL COMMENT '종료일', -- 종료일
	deadline_dt         DATE         NULL     COMMENT '총모금일수', -- 총모금일수
	goal                INTEGER      NOT NULL COMMENT '목표금액', -- 목표금액
	status              INTEGER      NOT NULL DEFAULT 1 COMMENT '상태', -- 상태
	note                TEXT         NULL     COMMENT '비고' -- 비고
)
COMMENT '모금함';
